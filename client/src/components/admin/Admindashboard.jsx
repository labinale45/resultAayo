...
export default function Admindashboard() {
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Student and Teacher Count ${selectedClass ? `for Class ${selectedClass}` : ''} (${new Date(dateRange.startDate).toLocaleDateString()} - ${new Date(dateRange.endDate).toLocaleDateString()})`,
        font: {
          size: 20,
          weight: 'bold'
        },
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const historyResponse = await fetch(
        `http://localhost:4000/api/auth/dashboard/history?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}${selectedClass ? `&class=${selectedClass}` : ''}`
      );
