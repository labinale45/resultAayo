...
const Setting = ({ onClose, role }) => {
  ...
  const handelSaveLogo = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4000/api/auth/settings/logo', {
        method: 'PUT',
        headers: {
      ...
      // Reset logo state after successful upload
      setLogo(null);
      setPreviewUrl(null);
  ...
  return (
<div className="w-[500px] dark:bg-[#253553] dark:text-white bg-white rounded-lg shadow-md p-6 max-w-md mx-auto settings-container px-9 py-7 max-h-[80vh] overflow-y-auto">      <div className="flex justify-between items-center mb-4">
